...
if (isset($_POST['upload'])) {
  if ($_FILES['file']['name']) {
      ...
      while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
          $batch = mysqli_real_escape_string($al, $data[0]);
          $st_name = mysqli_real_escape_string($al, $data[1]);
          $st_pin = mysqli_real_escape_string($al, $data[2]);
          $phone1 = mysqli_real_escape_string($al, $data[3]);
          $gender = mysqli_real_escape_string($al, $data[4]);

          // Insert into database using prepared statement
          $stmt = $al->prepare("INSERT INTO student (st_year, st_name, st_pin,password,gender, st_phone) VALUES (?, ?, ?, ?, ?, ?)");
          $stmt->bind_param("ssssss", $batch, $st_name, $st_pin, $st_pin, $gender, $phone1);
          $stmt->execute();
      }

      fclose($handle);
      ?><script>
      alert('File uploaded and students added successfully!');
...
{ 
	...
	$u=mysqli_query($al,"insert into student values('$batch','$st_name','$st_pin','$st_pin','$gender',$phone1)");
	if($u==true)
	{
		?>
      ...
      <form action="" method="post">
        <div class="user-details">
          ...
          <div class="input-box">
            ...
            <input type="text" placeholder="Enter Student Name" name="name1" required >
          </div>
          <div class="input-box">
            <span class="details">Reg No</span>
            <input type="text" placeholder="Enter Student Reg Number" name="pin" required>
          </div>
