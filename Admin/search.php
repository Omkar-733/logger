...
<div class="login">
      <form action="" method="POST">
        <div class="user-details" >
            ...
            <input type="text" placeholder="Enter your Batch.. Ex:2022" name="batch"  >
          </div>
               <div class="input-box">
            <span class="details">DATE</span>
            <input type="text" placeholder="Enter Date"  name="date" value="">
          </div>
          <div class="input-box">
              
              <span class="details">Student Name</span>
            <input type="text" placeholder="Enter Student Name" name="na">
          </div>
          <div class="input-box">
            <span class="details">REG NO</span>
            <input type="text" placeholder="Enter Student Reg Number" name="pin">
          </div>
          
       
          <div class="input-box">
            <span class="details">LAB NAME</span>
            <input type="text" placeholder="Enter lab name" name="lab_n" >
          </div>
          <div class="input-box">
            <span class="details">COMPUTER NO</span>
            <input type="text" name="com_no" placeholder="Enter computer no" value="">
          </div>
        </div>
...
<table bordercolor="white" border="0" cellpadding="10px" cellspacing="20" border="2">
    ...
    if (!empty($com)) {
        $query .= " AND (com_no='$com')";
    }
    if (!empty($lab)) {
        $query .= " AND lab_name='$lab'";
    }
    if (!empty($st_pin)) {
        $query .= " AND st_pin='$st_pin'";
    }
    if (!empty($na)) {
        $query .= " AND st_name='$na'";
    }
    
    $query .= " ORDER BY st_pin";
    $h = mysqli_query($al, $query);
    ...
    ?>
        <tr>
            ...
            <td align="center">
                <a href="delete1.php?del1=<?php echo $j['time']; ?>" onClick="return confirm('Are you sure?')" style="text-decoration:none;font-size:18px;color:rgba(255,0,4,1.00);">[x]</a>
            </td> 
        </tr>
    <?php } ?>
</table>
